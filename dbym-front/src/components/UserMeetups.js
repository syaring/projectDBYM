...
class UserMeetups extends Component {
  ...
  getUserInfo() {
    axios.get(`http://localhost:8080/meetups/${this.props.uid}`).then((data)=>{
      this.setState({
        meetupsList: data.data,
        isLoaded: true
      });
    });
  }
  ...
  setMyLocation(mid) {
    navigator.geolocation.getCurrentPosition((p)=> {
      if(p){
        axios.post(`http://localhost:8080/meetups/location/${mid}`, {
                  ...
                  <li key={index}>
                    ...
                    함께하는 친구 :
                    {
                      data.MemberList.map((mem, index) => {
                        return (
                        (mem.uid !== this.state.userId) ?
                          mem.name+'님 '  : ''
                        )
                      })
                    } <br/>
                    모임 장소 : {data.isAllInputSet ? data.meetupsPlace : <div>정하는중</div>}
